...
class RuntimeApp:
    ...
    def init(
        ...
        # 9. V1.5: Director (autonomous decision brain)
        from runtime.director import Director, ArtifactManager, create_director_hook
        ...
        # Register Director as AFTER_AGENT hook (runs after quality gate)
        self.hook_dispatcher.register(
            hook_events.AFTER_AGENT,
            create_director_hook(self.director),
        )
        logger.info("Hook registered: Director (AFTER_AGENT)")

        # 10. V1.5: A2A Conversation Bus
        from runtime.agent_conversation import AgentConversationBus
        self.conversation_bus = AgentConversationBus()
        logger.info("A2A Conversation Bus initialized")

        # 11. V1.5: Story Memory
        from runtime.memory.story_memory import StoryMemory
        self.story_memory = StoryMemory(memory_manager=self.memory_manager)
        logger.info("Story Memory initialized")
    ...
    def _log_init_stats(self):
        ...
        stats = {
            ...
            "hooks": ["structured_logger", "quality_gate", "director"],
    ...
    def get_workflow_runner(self):
        """Get a WorkflowEngine (V1.5) with Director + A2A + StoryMemory."""
